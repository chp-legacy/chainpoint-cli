...
async function executeAsync (yargs, argv) {
  ...
  // are we dealing with hashes or files?
  let useFilePaths = argv.file
  ...
  let params = []
  if (argCount < 2) { // there is no hash/path parameter
    output.addErrorResult({
      message: `missing hash argument`
    })
    output.display(quiet, json)
  } else if (argCount === 2) { // this is one hash/path, or a CSV of hashes/paths
    params = argv._[1].split(',')
  } else { // this is a list of multiple hashes/paths
  ...
  let workingParamSet = params.slice()
  while (workingParamSet.length > 0) paramSegments.push(workingParamSet.splice(0, POST_HASHES_MAX))

  for (let x = 0; x < paramSegments.length; x++) {
    submitTasks.push(async () => { return submitHashesAsync(hashDb, paramSegments[x], baseURI, useFilePaths) })
  }
  ...
  for (let x = 0; x < allResults.length; x++) {
    if (allResults[x].success) {
      ...
      if (useFilePaths) successResult.path = allResults[x].path
      output.addSuccessResult(successResult)
    } else {
      let errorResult = {}
      if (allResults[x].hash) errorResult.hash = allResults[x].hash
      errorResult.message = allResults[x].message
      if (useFilePaths) errorResult.path = allResults[x].path
      output.addErrorResult(errorResult)
    }
  }
  output.display(quiet, json)
}

async function submitHashesAsync (hashDb, paramArray, baseURI, useFilePaths) {
  let submitResults = []
  ...
  try {
    ...
    await retry(async bail => {
      ...
      firstAttempt = false
      if (useFilePaths) {
        submitResponse = await chp.submitFileHashes(paramArray, targetURI)
      } else {
        submitResponse = await chp.submitHashes(paramArray, targetURI)
      }
    }, {
  ...
  for (let x = 0; x < submitResponse.length; x++) {
    ...
    try {
      ...
      if (useFilePaths) submitResult.path = hashItem.path
